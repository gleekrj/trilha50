...
export function RegisterProfessionalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await apiPostJson<UserCreated>('/users', {
      name: name.trim(),
  ...
  return (
    <div className="flex flex-col items-center gap-8">
      <div className="flex items-center justify-center gap-[13px] p-2">
        <UserPlus className="size-7 shrink-0 text-black" strokeWidth={2} aria-hidden />
      ...
      >
        ...
        <div className="relative min-h-[38px] w-full max-w-[386px]">
          <input
            ...
            style={{ borderColor: '#E2DDD4', accentColor: brandTeal }}
          />
          <label
            htmlFor={termsId}
          ...
          >
            Li e aceito os{' '}
            <Link to="#" className="font-medium" style={{ color: brandTeal }}>
              termos de uso
            </Link>{' '}
            e a{' '}
            <Link to="#" className="font-medium" style={{ color: brandTeal }}>
      ...
      <p className="w-full text-center text-base font-bold tracking-[0.06em]">
        ...
        <Link to="/entrar" className="underline" style={{ color: brandTeal }}>
          Faça login
        </Link>
      </p>
    </div>
